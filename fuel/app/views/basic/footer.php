
			<!-- footer -->
			<footer class="footer clear">
				<!-- footer_contents -->
				<div class="footer_contents clearfix">
					<!-- Page Plugin -->
					<div class="facebook_page_plugin">

					</div>
					<!-- footer_contents_box -->
					<div class="footer_contents_box clearfix">
						<h5>mtoksuy.Photoについて</h5>
						<ul>
							<li><a href="<?php echo HTTP; ?>about/">mtoksuy.Photoについて</a></li>
						</ul>
					</div> <!--  -->

					<!--  -->
					<div class="footer_contents_box clearfix">
						<h5>mtoksuy.Photoをフォローする</h5>
						<ul>
					  	<li><a href="https://www.instagram.com/mtoksuy/" target="_blank">Instagram</a></li>
						</ul>
					</div> <!--  -->


				</div> <!-- footer_contents -->
				<!-- footer_bottom -->
				<div class="footer_bottom">
					<div class="footer_bottom_contents">
						<!-- コピーライト -->
						<section id="copy">
							<p class="m_0">&copy; <?php echo date('Y'); ?> <a href="<?php echo Uri::base(); ?>">mtoksuy.Photo</a></p>
						</section>
					</div>
				</div>
			</footer>  <!-- footer -->
